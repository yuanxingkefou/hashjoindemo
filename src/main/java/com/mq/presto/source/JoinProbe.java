...
public class JoinProbe
{
    ...
    public long getCurrentJoinPosition(JoinHash joinHash)
    {
...
//            long rawHash = BIGINT.getLong(probeHashBlock.get(), position);
            long rawHash =probeHashBlock.get().getLong(position);
            return joinHash.getJoinPosition(position, probePage, page, rawHash);
        }
        return joinHash.getJoinPosition(position, probePage, page);
    }
